...
valid_hists = []
# Safely assign color and add to the list if the histogram is valid
if hMtData:
    hMtData.SetLineColor(ROOT.kBlack)
    valid_hists.append(hMtData)
if hMtSignal:
    hMtSignal.SetLineColor(ROOT.kRed)
    valid_hists.append(hMtSignal)
if hMtBackground:
    hMtBackground.SetLineColor(ROOT.kBlue)
    hMtBackground.SetLineStyle(2)  # Dashed line for background
    hMtBackground.SetMarkerStyle(21) # Circle markers
    hMtBackground.SetMarkerSize(0.8)
    valid_hists.append(hMtBackground)

if valid_hists:
    # Use the first valid histogram to initialize the canvas drawing
    valid_hists[0].Draw("HIST")

    # Draw the rest of the histograms on the same canvas
    for h in valid_hists[1:]:
        h.Draw("HIST SAME")

    # Legend (only add entries for valid histograms)
    leg = ROOT.TLegend(0.65,0.7,0.88,0.88)
    if hMtData: leg.AddEntry(hMtData,"Data","l")
    if hMtSignal: leg.AddEntry(hMtSignal,"Signal MC","l")
    if hMtBackground: leg.AddEntry(hMtBackground,"Background MC","l")
    leg.Draw()

    c.SaveAs("/pythonn/results/transverseMassData_simple.pdf")
else:
    print "No valid histograms to draw. Please check file paths."
